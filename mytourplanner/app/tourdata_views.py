...
def tour_next_step(request, id=None):
    ...
    ins = tourdata_dbops.get_tourdata(id)

    if ins:

        tour_data = travelmode_dbops.fetch_travelmode_data(tour_data_id=int(id))

        travel_modes = {}

        for row in travelmode_dbops.fetch_travel_modes(tour_data_id=int(id)):

            travel_modes.update({
                str(row["id"]):{
                    "travel_name": cs.TRAVELMODE_CHOICES_DICT[row["travel_mode"]],
                    "from_to": row["from_to"]
                }
            })
        ...
        data.update({
            ...
            "travel_modes_json": json.dumps(travel_modes),
